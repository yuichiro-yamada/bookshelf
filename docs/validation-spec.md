...
| 説明 | 任意<br>最大1000文字 | 説明は1000文字以内で入力してください |
| 画像URL | 任意<br>URL形式<br>最大255文字 | URL形式で入力してください<br>画像URLは255文字以内で入力してください |
...
※ `authorize()`で`BookPolicy`を呼び出しているため、権限の判定は入力チェック（`rules()`）より先に行われる。更新（`PUT`）は書籍の登録者本人のみ許可され、他人の書籍に対しては入力内容にかかわらず403が返る（422にはならない）。新規登録（`POST`）はログインユーザーであれば許可される。
...
| コメント | 必須<br>最大255文字 | コメントを入力してください<br>コメントは255文字以内で入力してください |
...
| フォーム | バリデーションルール | エラーメッセージ |
|---|---|---|
| キーワード（`keyword`） | 任意<br>文字列<br>最大255文字 | キーワードは文字列で指定してください<br>キーワードは255文字以内で指定してください |
| ジャンルID（`genre`） | 任意<br>整数<br>genresテーブルのidに存在する | ジャンルIDは整数で指定してください<br>指定されたジャンルは存在しません |
| ページ番号（`page`） | 任意<br>整数<br>1以上 | ページ番号は整数で指定してください<br>ページ番号は1以上の値で指定してください |
| 取得件数（`per_page`） | 任意<br>整数<br>1以上100以下 | 取得件数は整数で指定してください<br>取得件数は1以上の値で指定してください<br>取得件数は100以下の値で指定してください |

...
| フォーム | バリデーションルール | エラーメッセージ |
|---|---|---|
| 書籍タイトル | 必須<br>最大255文字<br>unique | 書籍タイトルを入力してください<br>書籍タイトルは255文字以内で入力してください<br>この書籍タイトルはすでに登録されています |
...
| 出版日 | 必須<br>日付 | 出版日を入力してください<br>正しい出版日を入力してください |
| 説明 | 最大1000文字 | 説明は1000文字以内で入力してください |
| 画像URL | URL形式<br>最大2048文字 | URL形式で入力してください<br>画像URLは2048文字以内で入力してください |
...
※ `title`・`isbn`のunique判定は、更新時（`update`）のみ自分自身のレコードを除外して判定する（`Rule::unique(...)->ignore($book)`）。画面用・API用どちらのルートも、パラメータ名が`{book}`で揃っているためこの仕組みがそのまま動作する。
